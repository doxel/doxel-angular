...
.controller('LoginCtrl', function ($window, $scope, User, LoopBackAuth, $location, $sce, errorMessage, socketService, $q, $timeout, $compile, $state) {
  this.awesomeThings = [
    'HTML5 Boilerplate',
    'AngularJS',
    'Karma'
  ...
  */
  ...
  $scope.doSignup=function(really){
    ...
    $scope.getBrowserFingerprint(function(fingerprint){
      ...
      function(res) {
        ...
        LoopBackAuth.save();
        $state.transitionTo($scope.stateAfterSignin||'upload');
      },
      function(err){
        $timeout(function(){
  ...
  $scope.signin=function($event){
    ...
    function(res) {
      ...
      $state.transitionTo($scope.stateAfterSignin||'home');
    },
    function(err){
      console.log(err);
      alert('Login failed !');
